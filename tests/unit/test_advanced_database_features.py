...
from src.project_name.core.soft_delete_config import SoftDeleteConfig, get_soft_delete_config
...
class TestDatabaseHealthChecker:
    ...
    async def test_get_table_statistics(self, db_session: AsyncSession):
        ...
        await author_service.create({"name": "Active Author 1", "email": "active1@test.com"})
        await author_service.create({"name": "Active Author 2", "email": "active2@test.com"})
        
        # Create and soft delete some records
        deleted_author = await author_service.create({"name": "Deleted Author", "email": "deleted@test.com"})
...
class TestCascadingSoftDeleteManager:
    ...
    @pytest.mark.asyncio
    async def test_cascade_circular_reference_protection(self, db_session: AsyncSession):
...
class TestBaseService:
    ...
    async def test_get_statistics(self, db_session: AsyncSession):
        ...
        deleted_author = await service.create({"name": "Deleted", "email": "deleted@test.com"})
