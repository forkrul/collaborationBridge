...
class TestEnvironmentDetection:
    ...
    def test_environment_detection_from_env_var(self, mock_config_file, sample_config):
        ...
        sample_config["environments"]["staging"] = sample_config["environments"]["development"]
        
        config_json = json.dumps(sample_config)
        with patch("builtins.open", mock_open(read_data=config_json)):
            with patch("pathlib.Path.exists", return_value=True):
                with patch("src.project_name.utils.service_url_manager.settings.PROJECT_NAME", "project-name"):
    ...
    def test_environment_detection_fallback(self, mock_config_file):
        ...
        with patch("src.project_name.utils.service_url_manager.settings.ENVIRONMENT", "development"):
            manager = ServiceURLManager()
            assert manager.environment == "development"
