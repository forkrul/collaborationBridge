...
class PerformanceTestSuite:
    ...
    @pytest.mark.asyncio
    @pytest.mark.performance
    async def test_query_performance_with_soft_delete_filter(
        ...
        print(
            f"✅ Queried {len(result['items'])} active records in {query_time * 1000:.2f}ms"
        )
    ...
    async def test_cascading_delete_performance(self, db_session: AsyncSession):
        ...
        for i in range(num_companies):
            company = await company_service.create(
                {"name": f"Company {i}", "industry": "Technology"}
            )
        ...
        deleted_counts = await cascade_manager.cascade_soft_delete(
            companies[0], deleted_by="performance_test", reason="Performance testing"
        )
...
class ReliabilityTestSuite:
    ...
    async def test_concurrent_soft_delete_operations(self, db_session: AsyncSession):
        ...
        employee = await service.create(
            {"name": "Concurrent Test Employee", "email": "concurrent@test.com"}
        )
        ...
        async def soft_delete_task(deleted_by: str):
            try:
                return await service.soft_delete(
                    employee.id, deleted_by=deleted_by, reason="Concurrent test"
                )
    ...
    async def test_memory_usage_during_bulk_operations(self, db_session: AsyncSession):
        ...
        await soft_delete_manager.bulk_soft_delete(
            TestEmployee, employee_ids, deleted_by="memory_test", batch_size=batch_size
        )
...
class ComplianceTestSuite:
    ...
    async def test_audit_trail_completeness(self, db_session: AsyncSession):
        ...
        # Create employee
        employee = await service.create(
            {"name": "Audit Test Employee", "email": "audit@test.com"},
        ...
        await service.soft_delete(
            employee.id, deleted_by="compliance_officer", reason="GDPR deletion request"
        )
        ...
        print("   ✅ All audit information properly recorded")
    ...
    async def test_gdpr_compliance_features(self, db_session: AsyncSession):
        ...
        employee = await service.create(
            {"name": "GDPR Test User", "email": "gdpr@test.com"}
        )
        ...
        await db_session.refresh(employee)
        assert employee.deletion_reason.startswith(
            "User requested data deletion under GDPR"
        )

        print("\n🔒 GDPR Compliance Test:")
        print("   ✅ Deletion reason required and recorded")
        print("   ✅ Data protection officer identified")
        print("   ✅ GDPR Article 17 compliance maintained")
    ...
    async def test_data_integrity_during_operations(self, db_session: AsyncSession):
        ...
        company = await company_service.create(
            {"name": "Integrity Test Corp", "industry": "Testing"}
        )
        ...
        print("   ✅ Soft deleted records preserve relationships")
...
pytestmark = [pytest.mark.asyncio, pytest.mark.enhanced_testing]
