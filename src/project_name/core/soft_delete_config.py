...
class SoftDeleteConfig(BaseSettings):
    ...
    cascade_soft_delete: bool = Field(
        default=False,
        description="Enable cascading soft delete to related records"
    )
    ...
    enable_query_optimization: bool = Field(
        default=True,
        description="Enable automatic query optimization features"
    )
    ...
    require_deletion_reason: bool = Field(
        default=False,
        description="Require a reason for all soft delete operations"
    )
    
    require_deleted_by: bool = Field(
        default=True,
        description="Require identification of who performed the deletion"
    )
    ...
    enable_gdpr_compliance: bool = Field(
        default=False,
        description="Enable GDPR-compliant deletion options"
    )
    ...
    alert_on_high_deletion_ratio: bool = Field(
        default=True,
        description="Alert when deletion ratio exceeds threshold"
    )
    ...
    alert_on_bulk_operations: bool = Field(
        default=True,
        description="Alert on large bulk operations"
    )
    ...
    timezone: str = Field(
        default="UTC",
        description="Timezone for deletion timestamps"
    )
    
    enable_partial_indexes: bool = Field(
        default=True,
        description="Use partial indexes for better performance"
    )
    
    index_deleted_records: bool = Field(
        default=True,
        description="Create indexes on deleted record queries"
    )
    
    # Error Handling Settings
    retry_failed_operations: bool = Field(
        default=True,
        description="Retry failed soft delete operations"
    )
    ...
    skip_validation: bool = Field(
        default=False,
        description="Skip validation checks (for testing only)"
    )
    ...
    def get_cleanup_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "gdpr_retention_days": self.gdpr_hard_delete_after_days if self.enable_gdpr_compliance else None
        }
