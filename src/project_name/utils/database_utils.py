...
class DatabaseHealthChecker:
    ...
    async def check_soft_delete_indexes(
        ...
        try:
            ...
            result = await self.session.execute(index_query, {"table_name": table_name})
            ...
            results["error"] = f"Could not check indexes: {e!s}"
    ...
    async def get_table_statistics(
        ...
        try:
            ...
            return {
                "table": model_class.__tablename__,
                "error": f"Could not get statistics: {e!s}",
    ...
    async def get_health_report(
        self, model_classes: list[type[SoftDeleteModelType]] | None = None
    ) -> dict[str, Any]:
        ...
        if model_classes:
            for model_class in model_classes:
                ...
                table_report = {**table_stats, "indexes": index_status}
    ...
    async def _process_relationships(
        ...
        for relationship_prop in mapper.relationships:
            ...
            for related_instance in related_instances:
                if (
                    hasattr(related_instance, "soft_delete")
                    and not related_instance.is_soft_deleted
                ):
                    await self._cascade_delete_recursive(
                        related_instance,
                        deleted_by,
