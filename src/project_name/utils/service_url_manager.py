...
class ServiceURLManager:
    ...
    def __init__(self, environment: str | None = None, config_path: str | None = None):
    ...
    def _detect_environment(self) -> str:
        ...
        project_env = os.getenv(
            f"{settings.PROJECT_NAME.upper().replace('-', '_')}_ENV"
        )
        if project_env:
            return project_env
    ...
    def _build_service_base_url(
        self, env_config: dict[str, Any], service_config: dict[str, Any]
    ) -> str:
...
_default_manager: ServiceURLManager | None = None


def get_url_manager(environment: str | None = None) -> ServiceURLManager:
    ...
    if _default_manager is None or (
        environment and _default_manager.environment != environment
    ):
...
def get_service_url(service_name: str, environment: str | None = None) -> str:
...
def get_api_endpoint(
    category: str, endpoint: str, environment: str | None = None, **kwargs
) -> str:
