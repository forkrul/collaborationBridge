...
from src.project_name.i18n.config import SupportedLocale, LOCALE_CONFIGS
...
async def translate_text(
    ...
    current_locale: str = Depends(get_current_locale)
):
    ...
    if target_locale not in [locale.value for locale in SupportedLocale]:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported locale: {target_locale}"
        )
